...
session_start();
$_SESSION["current_page"] = "DNS_page.php";?>
        ...
        <div class="<?php echo $shelf ?>">
            ...
            <?php
                ...
                if(!isset($_GET['search'])){
                    ...
                    $query = "SELECT * FROM `inventory` WHERE `shelf` = '$shelf' AND `division` = 'DNS' ORDER BY `shelf`,`level`,`zone`,`depth` ";
                }

                // Search bar used: filter based on query
                else{
                    $val = $_GET['search'];
                    $query = "SELECT * FROM inventory WHERE CONCAT(`part_number`,`serial_number`,`name`) LIKE '%$val%' AND `shelf` = '$shelf' AND `division` = 'DNS' ORDER BY `shelf`,`level`,`zone`,`depth`";
                }
